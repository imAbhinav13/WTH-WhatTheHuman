...
LOGGER = logging.getLogger(
    "wth.stage3.7b.equivalence"
)

DEFAULT_QUESTION: Final = (
    "What is the feeling of self or personal identity?"
)
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "Pause between the two live provider-backed modes "
            "to reduce Groq TPM collisions."
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Print each exact/semantic/unclassified difference."
        ),
    )
...
def build_provider_bundle(
    *,
    ...
    groq_api_key = required_environment(
        "GROQ_API_KEY"
    )
...
def load_json_document(
    ...
    if not path.is_file():
        raise EquivalenceRunnerError(
            f"Expected artifact does not exist: {path}"
        )
...
def run_artifact_mode(
    *,
    ...
    LOGGER.info(
        "Stage 3.7B Artifact Mode starting"
    )
    ...
    run_phase15(
        ...
        evidence_package_path=(
            paths["phase14_evidence"]
        ),
        retrieval_manifest_path=(
            paths["phase14_manifest"]
        ),
        output_directory=layout.phase15,
        provider_config=(
            providers.domain_generation
        ),
        replace=True,
    )
    ...
    run_phase16(
        ...
        domain_responses_path=(
            paths["phase15_domains"]
        ),
        generation_manifest_path=(
            paths["phase15_manifest"]
        ),
    ...
    run_phase17(
        ...
        synthesis_path=(
            paths["phase16_synthesis"]
        ),
        synthesis_manifest_path=(
            paths["phase16_manifest"]
        ),
    ...
    run_phase18(
        ...
        coverage_path=(
            paths["phase17_coverage"]
        ),
        coverage_manifest_path=(
            paths["phase17_manifest"]
        ),
    ...
    return PipelineSnapshot(
        ...
        phase18=load_json_document(
            paths["phase18_response"]
        ),
    )
...
async def run_runtime_mode(
    *,
    ...
    LOGGER.info(
        "Stage 3.7B Runtime Mode starting"
    )

    google_api_key = required_environment(
        "GOOGLE_API_KEY"
    )
    ...
    phase15_result = await asyncio.to_thread(
        ...
        evidence_package=(
            phase14_result.evidence_package
        ),
        retrieval_manifest=(
            phase14_result.manifest
        ),
        provider_config=(
            providers.domain_generation
        ),
        output_paths={
            "science": (
                manifest_paths.phase15
                / "science_response.json"
            ).as_posix(),
            "advaita": (
                manifest_paths.phase15
                / "advaita_response.json"
            ).as_posix(),
            "samkhya": (
                manifest_paths.phase15
                / "samkhya_response.json"
            ).as_posix(),
            "combined": (
                manifest_paths.phase15
                / "domain_responses.json"
    ...
    phase16_result = await SynthesisService().synthesize(
        domain_responses=(
            phase15_result.domain_responses
        ),
        generation_manifest=(
            phase15_result.manifest
        ),
        provider_config=(
            providers.synthesis
        ),
        synthesis_output_path=(
            manifest_paths.phase16
            / "synthesis.json"
        ).as_posix(),
    )
    ...
    phase17_result = CoverageService().classify(
        ...
        synthesis=(
            phase16_result.synthesis
        ),
        synthesis_manifest=(
            phase16_result.manifest
        ),
        coverage_output_path=(
            manifest_paths.phase17
            / "coverage.json"
        ).as_posix(),
    )
    ...
    phase18_result = ResponseAssemblyService().assemble(
        ...
        coverage=(
            phase17_result.coverage
        ),
        coverage_manifest=(
            phase17_result.manifest
        ),
    )
...
def semantic_similarity(
    ...
    left_tokens = _tokenize(
        left_norm
    )
    right_tokens = _tokenize(
        right_norm
    )
    ...
    def visit(
        ...
        if path:
            ...
            if rule is not None:
                if (
                    rule.comparison
                    == EquivalenceClass.NON_DETERMINISTIC
                ):
                    results.append(
                        FieldComparison(
                            phase=phase,
                ...
                if (
                    ...
                    if left == right:
                        ...
                        detail = (
                            "Semantic field is also byte-equivalent."
                        )
                    else:
                        score = (
                            semantic_similarity(
                        ...
                        detail = (
                            ...
                            if score is not None
                                ...
                                f"{rule.rationale} "
                                "Values differ and require semantic review."
                            )
                        )
                ...
                if not (
                    ...
                    results.append(
                        FieldComparison(
                            ...
                            detail=(
                                "Field is present in only one mode."
                            ),
                        )
                    )
                    continue
                ...
                if (
                    ...
                    results.append(
                        FieldComparison(
                            ...
                            detail=(
                                "List element is present in only one mode."
                            ),
                        )
                    )
                    continue
        ...
        if left != right:
            results.append(
                FieldComparison(
                    ...
                    runtime_value=right,
                    detail=(
                        "Differing field is not yet classified "
                        "by Stage 3.7A policy."
                    ),
                )
            )
...
def structural_signature(
    ...
    if phase == "phase15":
        ...
        for domain in DOMAINS:
            ...
            for raw in claims:
                claim = _mapping_value(
                    raw
                )

                for ref in _sequence_value(
                    claim.get(
    ...
    sections = _mapping_value(
        document.get(
            "sections"
        )
    )

    domain_perspectives = _mapping_value(
        sections.get(
            "domain_perspectives"
        )
    )

    comparative = _mapping_value(
        sections.get(
    ...
    citations = _sequence_value(
        document.get(
            "claim_level_citations"
        )
    )

    fallback = _mapping_value(
        sections.get(
...
def compare_snapshots(
    *,
    ...
    return tuple(
        compare_phase(
            phase=phase,
            artifact=artifact.by_phase(
                phase
            ),
            runtime=runtime.by_phase(
                phase
            ),
        )
        for phase in phases
...
def report_document(
    ...
    return {
        "stage": (
            "stage_3_7b_artifact_runtime_equivalence"
        ),
        ...
        "phases": [
            {
                ...
                "passed": phase.passed,
                "exact_failures": (
                    phase.exact_failures
                ),
                "semantic_reviews": (
                    phase.semantic_reviews
                ),
                "ignored_fields": (
                    phase.ignored_fields
                ),
                "unclassified_fields": (
                    phase.unclassified_fields
                ),
                "structural_passed": (
                    phase.structural_passed
                ),
                "structural_details": list(
                    phase.structural_details
                ),
                "differences": [
                    {
                        "path": item.path,
...
def print_summary(
    ...
    *,
    ...
    print(
        "STAGE 3.7B — ARTIFACT / RUNTIME EQUIVALENCE"
    )
    print(
        "=" * 78
    )
    ...
    for phase in result.phases:
        ...
        if (
            ...
            and (
                not phase.passed
                or phase.semantic_reviews
                or phase.unclassified_fields
            )
        ):
            for item in (
                phase.field_results
                ...
                print(
                    f"  - {item.status:<15} "
                    f"{item.path} "
                    f"[{item.comparison_class}]"
                )

                if item.status == "FAIL":
                    print(
                        f"      artifact={item.artifact_value!r}"
                    )
                    print(
                        f"      runtime ={item.runtime_value!r}"
                    )
    ...
    print(
            ...
            "Semantic differences are review items and do not fail "
            "3.7B by themselves."
...
async def execute_equivalence(
    *,
    ...
    layout = ArtifactLayout.create(
        artifact_root
    )

    artifact_snapshot = run_artifact_mode(
        project_root=project_root,
...
def main() -> int:
    ...
    try:
        if persistent_root is not None:
            ...
            result = asyncio.run(
                execute_equivalence(
                    ...
                    providers=providers,
                    cooldown_seconds=max(0.0, arguments.cooldown_seconds),
                )
            )
        else:
        ...
        print_summary(result, show_differences=arguments.show_differences)

        if arguments.report_json is not None:
            write_report(arguments.report_json, report_document(result))
